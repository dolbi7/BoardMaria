...
.userid, .username, .ac, .telno, .email, .regdate, .lastlogindate, .pwmodidate, .address, .job, .hobby, .input_content {
    width: 90%;
    height:25px;
    outline:none;
    color: #636e72;
...
<div class="main" align="center">
    ...
    <div class="viewMemberInfo">
        <h1 style="text-align:center;">사용자 정보</h1>
         <center><div id="ImageView"><img src="/profile/${user.stored_filename}" style='width:300px; height:auto;'></div></center>
         <div class="userid">아이디 : <%=userid %></div>
		 <div class="username">이름 : ${user.username}</div>
         <div class="telno">전화번호 : ${user.telno} </div>
         <div class="email">이메일주소 : ${user.email}</div>
         <div class="address">주소 : [${user.zipcode}]${user.address}</div>
         <div class="job">직업 : ${user.job}</div>
         <div class="hobby">취미 : ${user.hobby}</div>
         <div class="input_content">자기소개 : ${user.description}</div>
    ...
    <div class="bottom_menu" align="center">
        ...
        <a href="/user/userInfoModify">기본정보 변경</a> &nbsp;&nbsp;
